...
function NotificationsPage() {
    ...
    const color = { success: '#d4edda', warning: '#fff3cd', info: '#cce5ff', error: '#f8d7da' };
    return (<div style={{ padding: '24px' }}>
      <h2>Notifications</h2>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {items.map(n => (<div key={n.id} style={{ padding: '12px 16px', borderRadius: '8px', background: color[n.type] }}>
            <strong style={{ textTransform: 'uppercase', fontSize: '11px' }}>{n.type}</strong>
            <p style={{ margin: '4px 0 0' }}>{n.msg}</p>
            <small><components_1.RelativeTime base={new Date(Date.now() - n.id * 300000).toISOString()}/></small>
          </div>))}
      </div>
    </div>);
}
